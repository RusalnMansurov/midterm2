...
export default function Portfolio({ prices, user }) {
  ...
  // Net Worth = cash + sum(shares × current live price)
  // Calculated entirely on client from WebSocket prices — NEVER stored in DB
  ...
  const handleSell = async (ticker, shares) => {
    try {
      await api.post('/trades/sell', { ticker, shares: 1 });
  ...
  return (
    <main style={{ maxWidth: 700, margin: '30px auto', padding: '0 16px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 20 }}>
        ...
        <div className="card">
          <div className="muted" style={{ marginBottom: 4 }}>Net Worth <span style={{ fontSize: 10, background: 'var(--bg3)', padding: '1px 5px', borderRadius: 6 }}>live</span></div>
